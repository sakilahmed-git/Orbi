...
| 3 | Real-data (e-STURT) ingestion | ⚠ Code written, NOT verified | `core/real_data.py` written against the actually-verified Zenodo record structure (DOI 10.5281/zenodo.14031911, confirmed real: 20 episode zips, 24.5GB total, CC BY-NC-ND 4.0 — non-commercial/no-derivatives, fine for hackathon validation, NOT for commercial resale of the data itself). Ground-truth loader (`load_shaker_log`) is trustworthy — folder/file structure and the "zeros = missing data" handling are confirmed from the dataset page itself. **Event-CSV column order in `load_camera_events` is an UNVERIFIED GUESS** — flagged in the code's own docstring — because the sandbox environment cannot reach zenodo.org (not on its network allowlist) to fetch a real sample. **BLOCKED until a human downloads a real sample and either uploads a trimmed excerpt or runs `inspect_raw_csv()` locally and reports back the real column order.** Recommended real file: episode 17 (`https://zenodo.org/records/14031911/files/17.zip?download=1`, 1.1GB — chosen because the paper's own Figs. 6-8 use sequence 17, confirmed non-corrupt). Do not mark this section "done" or claim real-data validation in the pitch until this loop is closed with actual bytes. |
| 4 | Classical beacon lock-on | ☐ Not started | |
| 5 | Learned discriminator | ☐ Not started | |
| 6 | Disturbance characterization | ☐ Not started | |
| 7 | Acquisition-envelope surrogate | ☐ Not started | |
